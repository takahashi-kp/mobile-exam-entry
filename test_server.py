...
class MergePayloadTests(unittest.TestCase):
    ...
    def test_frontend_and_versioned_script_are_served(self):
        ...
        self.assertIn(b"app.js?v=20260925-03", index.data)
        ...
        self.assertIn(b'id="entryGuidanceBadge"', index.data)
        self.assertIn(b'id="entryGuidanceResult"', index.data)
        self.assertIn('name="保健指導_保険"'.encode(), index.data)
        ...
        self.assertIn(b'id="entryGroupMenu"', index.data)
        ...
        self.assertIn('id="confirmEntryGroup" type="button" hidden>確認</button>'.encode(), index.data)
        ...
        self.assertIn(b"processBloodTubeBarcode", script.data)
        self.assertIn(b"getBloodConfirmationLog", script.data)
        self.assertIn(b"renderBloodConfirmationLog", script.data)
        self.assertIn(b"getBloodConfirmationLogFromData", script.data)
        self.assertIn("採血確認日時".encode(), script.data)
        self.assertIn(b"assignReceptionNumberIfEmpty", script.data)
        self.assertIn(b"focusPersonalNumberForNextPatient", script.data)
        self.assertIn(b"clearEntryRecordForm", script.data)
        self.assertIn(b'field.type === "checkbox" || field.type === "radio"', script.data)
        self.assertIn(b'bloodTubeBarcode.value = ""', script.data)
        self.assertIn(b'diagnosisReference.innerHTML = ""', script.data)
        self.assertIn(b'verificationPanel.hidden = true', script.data)
        self.assertIn("エラー　番号が一致しません".encode(), script.data)
        self.assertNotIn("再登録（仮保存）".encode(), script.data)
        self.assertNotIn("利用者確認済みとして最終確定しました".encode(), script.data)
